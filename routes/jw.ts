...
router.post('/tranfer', verifyuser , async (req: Request, res: Response) => {
    try {
        const { from, fromkey, to, amount } = req.body
        if(!from || !fromkey || !to || !amount){
        ...
        const signtransection = await web3.eth.accounts.signTransaction({
            ...
            gas: gas,
            gasPrice: gasPrice,
            data: contract.methods.transfer(to, realamount * 100000000).encodeABI(),
        }, fromkey);
        ...
        res.status(200).json(sendSignedTransaction.transactionHash)
    } catch (error) {
        res.status(403).json({
            error
        })
